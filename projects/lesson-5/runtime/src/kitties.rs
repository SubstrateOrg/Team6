...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		pub fn buy(origin,	kitty_id: T::KittyIndex, max_price: T::Balance) {
			...
			Self::do_buy(&sender, kitty_id, max_price)?;
		}
	}
}
...
impl<T: Trait> Module<T> {
	...
	fn do_transfer(sender: &T::AccountId, to: &T::AccountId, kitty_id: T::KittyIndex) -> Result {
		...
		//let kitty_linked_item = <OwnedKitties<T>>::get((sender.clone(), Some(kitty_id)));
		//ensure!(kitty_linked_item.is_some(), "sender is not kitty's owner");
		let owner = Self::owner_of(kitty_id).ok_or("No owner for this kitty")?;

		ensure!(owner == *sender, "sender is not kitty's owner");
	...
	fn do_buy(sender: &T::AccountId, kitty_id: T::KittyIndex, max_price: T::Balance) -> Result {
		let kitty = Self::kitty(kitty_id);
		...
		ensure!(kitty_price > 0.into(), "The cat you want to buy is not for sale");
